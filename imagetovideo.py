...
img = cv2.imread('seq0_0000_2.jpg')  #读取第一张图片
fps = 25
imgInfo = img.shape
size = (imgInfo[1],imgInfo[0])  #获取图片宽高度信息
print(size)
fourcc = cv2.VideoWriter_fourcc(*"MJPG")
videoWrite = cv2.VideoWriter('camera_three_rgb.mp4',fourcc,fps,size)# 根据图片的大小，创建写入对象 （文件名，支持的编码器，5帧，视频大小（图片大小））
#videoWrite = cv2.VideoWriter('0.mp4',fourcc,fps,(1920,1080))
dir=r'..\rgbd_people_unihall.tar\rgbd_people_unihall\mensa_seq0_1.1\rgb_jpg_camera_three'
